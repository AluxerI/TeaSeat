...
class Sub_SubcategoryFactory extends Factory
{
    public function definition(): array
    {
        ...
        if (str_contains($subcategory->name, 'чай')) {
            ...
            $name = $this->faker->unique(false)->randomElement($names) . ' чай';
        } 
        // Для сладостей
        else {
            $names = ['Классическое', 'Шоколадное', 'Фруктовое'];
            $name = $this->faker->unique(false)->randomElement($names) . ' ' . $subcategory->name;
        }
