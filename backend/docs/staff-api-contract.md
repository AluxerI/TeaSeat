...
| Сборщик упаковал складскую часть в точке консолидации | picker | `packed` |
| Все части готовы к клиентской доставке | backend/picker | `ready_for_delivery` |
...
сборщик точки назначения подтверждает приём и переводит часть в `packed`.
`delivered` используется только для фактической доставки клиенту. `pickup` и `external` не попадают
