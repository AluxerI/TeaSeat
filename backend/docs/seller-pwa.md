...
Пострадавший интернет-заказ заранее не выбирается. Менеджерский API динамически
возвращает актуальные заказы-кандидаты из онлайн-резервов. Окончательный выбор
остаётся за менеджером; контракт описан в
[manager-fulfillment.md](manager-fulfillment.md).
