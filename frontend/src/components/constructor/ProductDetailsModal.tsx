...
export default function ProductDetailsModal({ box, size, cellSizeMm, onClose, onAdd, atLimit, remainingStock }: Props) {
  const dialog = useRef<HTMLDialogElement>(null);
  const product = size.product;
  const itemWeight = constructorItemWeight(size);
  const insufficientStock = remainingStock < size.product_quantity;
  ...
  return <dialog ref={dialog} className={styles.productDetailsModal} onCancel={(event) => { event.preventDefault(); onClose(); }}
    ...
    <div className={styles.productDetailsBody}>
      ...
      <dl className={styles.productDetailsGrid}>
        ...
        <div><dt>Вес позиции</dt><dd>{itemWeight === null ? "Не указан" : `${itemWeight.toLocaleString("ru-RU")} г`}</dd></div>
        <div><dt>Доступно на сайте</dt><dd data-stock-empty={insufficientStock}>{constructorStockLabel(size, remainingStock)}</dd></div>
      ...
      <div className={styles.productDetailsActions}>
        ...
        <IconButton className={styles.addButton} title={insufficientStock ? "Недостаточно товара" : "Добавить в коробку"} aria-label="Добавить формат в коробку" disabled={atLimit || insufficientStock} onClick={() => { onAdd(size); onClose(); }}><AddShoppingCartRounded /></IconButton>
      </div>
    </div>
  </dialog>;
}
