...
describe("App routing: staff sections are protected", () => {
  ...
  it("opens courier PWA only with courier permission", async () => {
    cacheUser("view assigned deliveries");
    window.history.pushState({}, "", "/courier");
    render(<App />);
    expect(
      await screen.findByRole("heading", { name: "Свободные доставки" })
    ).toBeInTheDocument();
    expect(window.location.pathname).toBe("/courier");
  });
